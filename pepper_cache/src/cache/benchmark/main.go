...
func operate(id, count int, ch chan *result) {
	client := clients.New(typ, server, port)
	...
	for i := 0; i < count; i++ {
		...
		c := &clients.Cmd{name, key, value, nil}
		if pipelen > 1 {
			cmds = append(cmds, c)
			if len(cmds) == pipelen {
...
var typ, server, operation, port string
...
func init() {
	...
	flag.StringVar(&port, "p", "9004", "port")
