...
func (r *redisClient) set(key, value string) error {
	return r.Set(key, value, 5*time.Minute).Err()
}
...
func newRedisClient(server, port string) *redisClient {
	return &redisClient{redis.NewClient(&redis.Options{Addr: server + ":" + port, Password: "********", ReadTimeout: -1})}
}
